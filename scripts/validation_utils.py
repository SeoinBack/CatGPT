...
CrystalNNFP = CrystalNNFingerprint.from_preset("cn")
CompFP = ElementProperty.from_preset('magpie')
TOKENIZER = PreTrainedTokenizerFast.from_pretrained('/home/ahrehd0506/git/CatPT/data/tokenizer/3f_tokenizer/',1024)
...
def ORR_validity(structure):
    philic = ['Al','Sc','Ti','V','Mn','Fe','Co','Ni','Cu','Zn','Ga','Y','Zr','Nb','Mo',
        'Ru','Rh','Pd','Pb','Cd','In','Sn','Sb','Te','Hf','Os','Ir','Pt','Bi']
    phobic = ['S','Se','Ag','Au','Hg','Tl']

    atoms = AseAtomsAdaptor.get_atoms(structure)
    compos_dict = structure.composition.as_dict()
    keys = set(compos_dict.keys())
    
    active_site_symbol = atoms[find_active_site_ORR(atoms)].symbol
    
    philic_set = set.intersection(keys,set(philic))
    phobic_set = set.intersection(keys,set(phobic))
    
    adsorbate_validity = True
    composition_validity = True
    active_site_validity = True
    
    
    if (compos_dict['O'] != 2.) or (compos_dict['H'] != 1.):
        adsorbate_validity = False
    if len(keys) != 4:
        composition_validity = False
    if active_site_symbol not in philic:
        active_site_validity = False
    if not (len(philic_set) == 1) & (len(phobic_set) == 1):
        composition_validity = False
    
    validity = adsorbate_validity & composition_validity & active_site_validity
    return composition_validity, adsorbate_validity, active_site_validity, validity

def catalyst_validity(crystal):
    device = 'cuda' if torch.cuda.is_available() else 'cpu'

    reward_model = BertForSequenceClassification.from_pretrained(
        '/home/ahrehd0506/git/CatGPT/models/oc20-2M-BERT/',
    ).to(device)
    atoms_str = atoms_to_str(AseAtomsAdaptor.get_atoms(crystal))
    input_ = TOKENIZER(atoms_str,return_tensors='pt').to(device)
    score = reward_model(**input_)['logits'].detach().cpu().numpy()[0][0]
...
class GenEval(object):
    ...
    def get_validity(self):
        ...
        catal_valid = np.array([c.catal_valid for c in self.crys]).mean()
        valid = np.array([c.valid for c in self.crys]).mean()
        return {'comp_valid': comp_valid,
                'struct_valid': struct_valid,
                'catal_valid': catal_valid,
                'valid': valid}
    ...
    def get_catal_score(self):
        return {'catal_score' : np.mean([c.catal_score for c in self.valid_samples])}
    ...
    def get_metrics(self):
        ...
        metrics.update(self.get_catal_score())
...
class Crystal(object):
    def __init__(self, structure):
        ...
        self.comps = structure.composition
        self.get_validity()
        self.get_fingerprints()

    def get_validity(self):
#        self.valid = True
        self.comp_valid = smact_validity(self.structure)
        try:
            self.struct_valid = structure_validity(self.structure)
        except ValueError:
            self.struct_valid = False
        self.catal_valid, self.catal_score = catalyst_validity(self.structure)
        self.valid = self.struct_valid and self.catal_valid
        

    def get_fingerprints(self):
        self.comp_fp = CompFP.featurize(self.comps)
        try:
...
def find_active_site_ORR(atoms):
    oxygen_idx = [i for i, atom in enumerate(atoms) if atom.symbol == 'O']
    hydro_idx = [i for i, atom in enumerate(atoms) if atom.symbol == 'H']
    ads_idx = oxygen_idx + hydro_idx

    lower_oxygen_idx = oxygen_idx[np.argmin([i[2] for i in atoms.get_scaled_positions()[oxygen_idx]])]
    lower_oxygen_dist = atoms.get_all_distances(mic=True)[lower_oxygen_idx]
    lower_oxygen_dist[ads_idx] += 10

    active_site_idx = np.argmin(lower_oxygen_dist)
    return active_site_idx